...
export function WishlistIcon(props) {
  const { clothId, className, removeCallback } = props;
  ...
  useEffect(() => {
    (async () => {
      try {
        const response = await wishlistCtrl.check(user.id, clothId);
  ...
  const addWishlist = async () => {
    const response = await wishlistCtrl.add(user.id, clothId);
    setHasWishlist(response);
  };
